...
public class ReleaseServiceImpl implements ReleaseService{
    ...
    @Override
    public String deleteRelease(long id) throws ReleaseDetailsNotFoundException {
        log.info("Inside the ReleaseServiceImpl.deleteRelease, id:{}", id);
        Optional<Release> releaseOptional = releaseRepository.findById(id);
        if(releaseOptional.isEmpty()){
            throw new ReleaseDetailsNotFoundException("Invalid release id");
        }

        releaseRepository.delete(releaseOptional.get());
        log.info("End of ReleaseServiceImpl.deleteRelease");
        return "Release details has been deleted";
    }
